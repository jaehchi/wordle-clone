...
import { MAX_BOARD, MAX_CHARS, EVAL_DELAY, GAME_WON_DELAY } from './lib/settings';
import { NOT_ENOUGH_CHARS, NOT_IN_WORD_LIST } from './lib/strings';
import { GameStatus } from './util/types';
import './App.css';

export const App = () => {
  const [currentGuess, setCurrentGuess] = useState('');
  const [solution, setSolution] = useState(() => loadGameState().solution || generateNewSolution());
  const [board, setBoard] = useState(() => {
    const state = loadGameState();
    return state.solution === solution ? state.board : [];
  });
  const [gameStatus, setGameStatus] = useState<GameStatus>(() => loadGameState().gameStatus || 'ONGOING');
