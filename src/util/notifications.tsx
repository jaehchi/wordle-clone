...
export const notifyError = (message: string) => {
  toast.remove();
  const classname =
    'bg-red-500 px-6 py-3 flex justify-center text-white font-bold tracking-wider rounded-lg';
...
export const notifyGameOver = () => {
  ...
  const classname =
    'bg-black px-6 py-3 flex justify-center text-white font-bold tracking-wider rounded-lg';
  const options = { duration: isGameWon ? 10000 : Infinity };
  const message = isGameWon
    ? WINNING_MESSAGES[attempts - 1]
